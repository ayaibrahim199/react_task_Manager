    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-sm p-8 space-y-6 bg-white rounded-lg shadow-xl">
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-4">
          <div>
            ...
            <input 
              ...
              className={`w-full px-3 py-2 mt-1 border rounded-md shadow-sm bg-white text-gray-800 placeholder:text-gray-500 focus:outline-none focus:ring-blue-500 focus:border-blue-500 ${errors.name ? 'border-red-500' : 'border-gray-300'}`} 
            />
          ...
          <button 
            type="submit"
            disabled={loading}
            className={`w-full px-4 py-2 text-white rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 transition duration-150 ${loading ? 'bg-green-300 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700 focus:ring-green-500'}`}
          >
        ...
        <p className="text-center text-sm text-gray-600">
          Already have an account? <Link to="/" className="font-medium text-blue-600 hover:text-blue-500">Login here</Link>
        </p>
      </div>
    </div>
  );
